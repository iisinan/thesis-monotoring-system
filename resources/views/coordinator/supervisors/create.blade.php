        ...
        <div class="lg:col-span-1">
            <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/40 p-8 md:p-10 sticky top-10">
                ...
                <div class="p-4 bg-slate-50 rounded-2xl border border-slate-100 mb-6">
                    ...
                    <code class="text-xs font-mono font-bold text-acetel-600 px-2.5 py-1 bg-white rounded-lg border border-acetel-100 block truncate">name, email, expertise</code>
                </div>

                <form action="{{ route('coordinator.supervisors.bulkStore') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    @csrf
                    <div>
                    ...
                    <button type="submit" class="w-full px-6 py-3.5 bg-white border border-slate-200 shadow-sm rounded-2xl text-xs font-black text-slate-700 uppercase tracking-widest hover:bg-slate-50 hover:border-slate-300 transition-all flex justify-center items-center gap-2 group">
