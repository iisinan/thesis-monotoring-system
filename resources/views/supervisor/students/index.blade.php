...
<div class="space-y-8 animate-in-up">
    ...
    <!-- Filter/Search Area (Placeholder for consistency) -->
    <div class="flex flex-col sm:flex-row items-center justify-between gap-4 glass p-4 rounded-3xl border border-white/10">
        <div class="relative w-full sm:w-96">
            ...
            <input type="text" placeholder="Search portfolio..." class="w-full bg-white/5 border-none rounded-2xl pl-12 pr-4 py-3 placeholder-slate-400 text-sm focus:ring-2 focus:ring-primary-500/20 text-slate-900 font-medium">
        </div>
        <div class="flex items-center gap-2">
            <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest px-4">Sort by Activity</span>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 gap-6">
        @forelse($assignments as $assignment)
            <div class="group relative glass p-8 rounded-[2.5rem] border border-slate-100 hover:border-primary-200 hover:shadow-premium transition-all duration-500 hover:-translate-y-1">
                <div class="flex flex-col lg:flex-row items-center gap-8">
                    <!-- Identity Section -->
                    <div class="flex items-center gap-8 flex-1 w-full min-w-0">
                        <div class="relative">
                            <div class="w-24 h-24 rounded-3xl bg-acetel-50 border border-acetel-100 flex items-center justify-center text-acetel-600 text-4xl font-black shadow-inner group-hover:bg-acetel-600 group-hover:text-white group-hover:rotate-3 transition-all duration-500 overflow-hidden">
                        ...
                        <div class="min-w-0 flex-1">
                            <div class="flex flex-wrap items-center gap-4 mb-3">
                                ...
                                <div class="flex items-center gap-2">
                                    <x-badge type="success" label="On Track" />
                            ...
                            <p class="text-slate-600 font-medium text-base mb-4 line-clamp-1 italic italic">"{{ $assignment->thesis->title }}"</p>
                            
                            <div class="flex flex-wrap items-center gap-6">
                                <div class="flex items-center gap-2">
                                    <span class="w-2 h-2 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]"></span>
                                    <span class="text-[10px] font-black text-emerald-600 uppercase tracking-[0.2em] uppercase">Phase: {{ $assignment->thesis->currentMilestone->template->name ?? 'Completed/Archived' }}</span>
                                </div>
                                <div class="flex items-center gap-2 text-slate-400">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                    <span class="text-[10px] font-black uppercase tracking-widest">Assigned {{ $assignment->created_at->format('M d, Y') }}</span>
                                </div>
                    ...
                    <div class="flex items-center gap-4 w-full lg:w-auto lg:pl-10 lg:border-l border-slate-100/50">
                        <div class="flex flex-col items-center px-6">
                            ...
                            <p class="text-2xl font-black text-primary-600 tracking-tighter">{{ $assignment->thesis->progress_percentage }}%</p>
                        </div>
                        <a href="{{ route('theses.show', $assignment->thesis) }}" class="flex-1 lg:flex-none flex items-center justify-center gap-3 px-8 py-5 bg-white border-2 border-slate-100 rounded-[1.5rem] text-slate-900 hover:bg-grad-premium hover:text-white hover:border-transparent transition-all duration-500 shadow-sm group/btn group-hover:shadow-acetel-500/10">
        ...
        @empty
            <div class="text-center py-24 bg-slate-50/50 border-2 border-dashed border-slate-200 rounded-[2.5rem]">
                ...
                <p class="text-slate-500 font-medium">No research students have been assigned to you yet.</p>
            </div>
        @endforelse
    </div>
</div>
